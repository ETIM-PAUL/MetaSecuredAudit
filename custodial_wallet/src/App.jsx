...
function App() {
  ...
  async function createWallet() {
    ...
    try {
      ...
      const tx = await FactoryContract.deployWallet(generateRandom10DigitNumber());
      await tx.wait();
      console.log(tx)
    ...
    <div className='h-screen'>
      ...
      {childContract !== null &&
        <div className='mt-6 w-fit mx-auto'>
          <div className="tabs tabs-boxed">
            <span onClick={() => setViewTab(true)} className={`tab ${viewTab && "tab-active"}`}>Deposit</span>
      ...
      {(childContract !== null && viewTab) &&
        <div className='mt-10 w-full md:max-w-xl mx-auto'>
          <div className="flex gap-3 flex-col justify-center w-full mx-auto md:gap-10">
            <div className="w-full">
              <label htmlFor="" className="font-normal text-[17px] leading-5 tracking-[0.5%] ">Amount to deposit</label>
              <div className="flex items-center border-[1px] border-[#696969] rounded-lg pl-[10px] gap-4 h-[72px] mt-2">
                <p className="font-normal head2 text-[16px] leading-[32px] ">ETH</p>
                <input value={amountVal} onChange={(e) => setAmountVal(e.target.value)} type="text" placeholder="0.00" className="bg-white text-[36px] leading-[53.2px] text-[#696969] h-[100%] w-fit outline-none rounded-r-lg" />
              </div>
          ...
          {/* button */}
          <div className="flex justify-center mt-4">
            <button disabled={depositState} onClick={() => depositFund()} className={`${depositState && "hover:cursor-not-allowed text-white"} btn btn-md w-24 md:w-auto text-white`}>
              {depositState &&
                <span className="loading loading-spinner loading-sm text-white"></span>
              }
